...
class UserController extends HomeController
{
    ...
    /* 注册页面 */
    public function spbcnRegister($username = '',$nickname='', $password = '', $email = '', $type = 'start')
    {
        $type = op_t($type);
        if (!C('USER_ALLOW_REGISTER')) {
    ...
    /* 登录页面 */
    public function spbcnLogin($username = '', $password = '')
    {
        $remember = '';
        if (IS_POST) { //登录验证
            /* 调用UC登录接口登录 */
            ...
            if (0 < $uid) { //UC登录成功
                /* 登录用户 */
                $Member = D('Member');
                $nickname = $Member -> where('uid='.$uid) -> getField('nickname');
                if ($Member->login($uid, $remember == 'on')) { //登录用户
                    //TODO:跳转到登录前页面
