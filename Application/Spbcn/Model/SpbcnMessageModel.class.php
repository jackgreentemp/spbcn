...
class SpbcnMessageModel extends Model
{
    /**获取全部没有提示过的消息
     * @param $uid 用户ID
     * @return mixed
    ...
    {
        $this->where('to_uid=' . $uid . ' and  is_read=0')->setField('is_read', 1);
    }
    ...
    {
        $messages = $this->where('to_uid=' . $uid . ' and  is_read=0 ')->order('id desc')->limit(99999)->select();
//        foreach ($messages as &$v) {
//            $v['ctime'] = friendlyDate($v['create_time']);
//            $v['content'] = op_t($v['content']);
//        }
//        unset($v);
    ...
    public function getAllMessageSended($userId){
        ...
        return $this->where(array('from_uid' => $userId)) -> order('id desc') ->select();
